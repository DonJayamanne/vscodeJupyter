...
export class NotebookFactory extends EventEmitter {
	...
	private async startNotebook(): Promise<Notebook> {
		...
		port = await getFreePort({ host: 'localhost', port });
		if (portIndex >= 0) {
		...
		try {
			await this.startJupyterNotebookInTerminal(startupFolder, args)
			...
			console.error('Jupyter Extension (startNotebook)', ex);
			throw new Error(`Failed to detect Jupyter Notebook. Please use 'Select Jupyter Notebook' command`);
		}
	}
}
